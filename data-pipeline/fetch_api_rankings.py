...
def ensure_players_exist(events):
    ...
    for event in events:
        ...
        player1_name = event.get('homeTeam', {}).get('name', event.get('homeTeam', {}).get('shortName', 'Unknown'))
        
        player2_id = event.get('awayTeam', {}).get('id')
        player2_name = event.get('awayTeam', {}).get('name', event.get('awayTeam', {}).get('shortName', 'Unknown'))
    ...
    for player_id in missing_player_ids:
        ...
        players_to_insert.append({
            ...
            'gender': player_info['gender'] or 'M'  # Default to 'M' if gender not available
        })
    ...
    for i in range(0, len(players_to_insert), batch_size):
        ...
        try:
            ...
            print(f"  ✓ Added batch of {len(batch)} players")
        except Exception as e:
            error_count += len(batch)
            print(f"  ✗ Error adding batch: {e}")
            
            # Try inserting one by one for this batch to identify issues
            for player in batch:
                try:
                    supabase.table('players').insert(player).execute()
                    added_count += 1
                    error_count -= 1
                    print(f"    ✓ Added: {player['name']} (ID: {player['player_id']})")
                except Exception as e2:
                    print(f"    ✗ Failed to add {player['name']} (ID: {player['player_id']}): {e2}")
